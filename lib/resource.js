...
function Resource(options, schema) {
	...
	_.each(options.resourceDefinition.actions,
		_.bind(function processAction(action) {
...
Resource.prototype.createAction = function (actionDefinition, isManaged) {
	...
	const invokeActionPromise = self => (...args) => {
		return new Promise((resolve, reject) => {
			const invokeActionBound = invokeAction.bind(self.apiResourceContext);
			return invokeActionBound(...args, (err, res) => {
				if (err) {
					return reject(err);
				}
				return resolve(res);
	...
	this[`${fnName}Promise`] = invokeActionPromise(this);
};
